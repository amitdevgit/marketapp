...
@section('title', 'Edit Customer Bill')
...
<div class="mb-8">
    <div class="flex items-center">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Customer Bill #{{ $customerBill->id }}</h1>
            <p class="mt-2 text-sm text-gray-600">Update customer bill details and items.</p>
        </div>
    </div>
</div>
@endsection
...
<div class="max-w-full mx-auto px-4">
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form id="customerBillForm" action="{{ route('customer-bills.update', $customerBill) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select 
                    id="customer_id" 
                    name="customer_id" 
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 @error('customer_id') border-red-500 @enderror"
                    required
                >
                    <option value="">Select Customer</option>
                    @foreach($customers as $customer)
                        <option value="{{ $customer->id }}" {{ old('customer_id', $customerBill->customer_id) == $customer->id ? 'selected' : '' }}>
                            {{ $customer->name }} - {{ $customer->phone }} ({{ ucfirst($customer->customer_type) }})
                        </option>
                    @endforeach
                </select>
                @error('customer_id')
            ...
            <div>
                ...
                <input 
                    ...
                    id="bill_date" 
                    name="bill_date" 
                    value="{{ old('bill_date', $customerBill->bill_date->format('Y-m-d')) }}"
            ...
            <div>
                <div class="flex items-center justify-between mb-6">
                    ...
                    <button type="button" id="addItemBtn" 
                            style="background-color: #2563eb !important; color: white !important; padding: 12px 24px !important; border-radius: 8px !important; font-weight: 500 !important; display: inline-flex !important; align-items: center !important; cursor: pointer !important; border: none !important; font-size: 14px !important; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05) !important;"
                            onmouseover="this.style.backgroundColor='#1d4ed8' !important; this.style.boxShadow='0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)' !important;"
                            onmouseout="this.style.backgroundColor='#2563eb' !important; this.style.boxShadow='0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)' !important;">
            ...
            <div>
                ...
                >{{ old('notes', $customerBill->notes) }}</textarea>
                @error('notes')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <div class="flex justify-between items-center">
                    ...
                    <span id="totalAmount" class="text-2xl font-bold text-blue-600">₹{{ number_format($customerBill->total_amount, 2) }}</span>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const products = @json($customerBill->items->map(function($item) {
        return [
            'id' => $item->product->id,
    ...
    const existingItems = @json($customerBill->items);
    let itemIndex = 0;
    ...
    function addItemRow(existingItem = null) {
        ...
        itemRow.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap w-1/2">
                <div class="relative">
                    <input type="text" class="product-search w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white" placeholder="Search or select product..." autocomplete="off" value="${existingItem ? `${existingItem.product.name} (${existingItem.product.unit})` : ''}">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right w-20">
                <input type="number" name="items[${itemIndex}][quantity]" step="0.01" min="0" class="w-16 px-2 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-right quantity-input" placeholder="0.00" value="${existingItem ? existingItem.quantity : ''}" required>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right w-20">
                <input type="number" name="items[${itemIndex}][rate]" step="0.01" min="0" class="w-16 px-2 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-right rate-input" placeholder="0.00" value="${existingItem ? existingItem.rate : ''}" required>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right w-20">
                <input type="number" name="items[${itemIndex}][misc_adjustment]" step="0.01" class="w-16 px-2 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-right misc-input" value="${existingItem ? existingItem.misc_adjustment : '0'}" placeholder="0.00">
            </td>
        ...
        function calculateTotal() {
            ...
            const netQuantity = quantity - misc; // Subtract misc adjustment
