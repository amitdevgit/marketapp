...
<div class="mb-8">
    <div class="flex items-center">
        ...
        <div>
            ...
            <p class="mt-2 text-sm text-gray-600">Update customer information.</p>
        </div>
    </div>
</div>
@endsection
    ...
    <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            >
                ...
                <option value="restaurant" {{ old('customer_type', $customer->customer_type) == 'restaurant' ? 'selected' : '' }}>Restaurant</option>
            </select>
            @error('customer_type')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
